...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(!uye) return message.channel.send(embed.setDescription("bir üye belirtmelisin!")).then(x => x.delete({timeout: 5000}));
  ...
  if (conf["isim-yas"]) {
    ...
    yazilacakIsim = `${uye.user.username.includes(conf.tag) ? conf.tag : (conf.ikinciTag ? conf.ikinciTag : (conf.isimtag || ""))} ${isim} | ${yaş}`;
  } 
  
  else {
    let isim = args.join(' ');
    if(!isim) return message.channel.send(embed.setDescription("Geçerli bir isim belirtmelisin!")).then(x => x.delete({timeout: 5000})); 
    yazilacakIsim = `${uye.user.username.includes(conf.tag) ? conf.tag : (conf.ikinciTag ? conf.ikinciTag : (conf.isimtag || ""))} ${isim}`;
  };
  
      if (conf.teyitsizRolleri && conf.teyitsizRolleri.some(rol => uye.roles.cache.has(rol))) kdb.add(`teyit.${message.author.id}.erkek`, 1);
      await uye.roles.add(conf.erkekRolleri || []).catch();
      setTimeout(() => {
      uye.roles.remove(conf.teyitsizRolleri || []).catch();
    }, 500)
    setTimeout(() => {
      uye.roles.remove(conf.kizRolleri || []).catch();
    }, 1000)
    setTimeout(() => {
    ...
    if (!eski) {
    message.channel.send(embed.setDescription(`<@${uye.user.id}> başarıyla ${yazilacakIsim} olarak kayıt edildi`)).then(x => x.delete({timeout: 15000}));
  }
  if (eski) {
    message.channel.send(embed.setDescription(`<@${uye.user.id}> başarıyla ${yazilacakIsim} olarak kayıt edildi \n 
